...
class Console {
    static log(...data) {
        if (Globaldata.logLevel == LogType.None ||
            ...
            Globaldata.logLevel == LogType.Warn_Error)
            return;
        console.log(data);
    }
    static warn(...data) {
        if (Globaldata.logLevel == LogType.None ||
            ...
            Globaldata.logLevel == LogType.Error ||
            Globaldata.logLevel == LogType.Log_Error)
            return;
        console.warn(data);
    }
    static error(...data) {
        if (Globaldata.logLevel == LogType.None ||
            Globaldata.logLevel == LogType.Log ||
            Globaldata.logLevel == LogType.Warn ||
            Globaldata.logLevel == LogType.Log_Warn)
            return;
        console.error(data);
    }
}
...
class Helper {
    ...
    */
    ...
    static getArcPoints(center, radius, count, startAngle, endAngle) {
        let points = [];
        let aoa = endAngle - startAngle; //角度差
        let amount = Math.round(360 * count / aoa); //把圆分成这些份数
...
class Guide {
    ...
    static async startGuide(targetVec, callback) {
        ...
        this.guideIntervalId = TimeUtil.setInterval(() => {
            ...
            if (mw.Vector.distance(this.preVec, currentVec) < 10)
                return;
        ...
        if (points.length >= this.guideEffectIds.length) {
            for (let i = 0; i < this.guideEffectIds.length; ++i) {
                EffectService.getEffectById(this.guideEffectIds[i]).then((effect) => {
                    if (effect)
                        effect.worldTransform.position = points[i];
                });
            }
...
class GameConfig {
    ...
    */
    ...
    ;
    static get PickUpGun() { return this.getConfig(PickUpGunConfig); }
    ;
    static get Role() { return this.getConfig(RoleConfig); }
    ;
    static get Skill() { return this.getConfig(SkillConfig); }
    ;
...
/*
 * @Author: jincheng.fang
 * @Date: 2023-01-19 09:40:03
 * @LastEditors: jincheng.fang
 * @LastEditTime: 2023-02-09 14:06:29
 * @FilePath: \commonmodule_dress\JavaScripts\module\dressModule\TouchScript.ts
 * @Description: 修改描述
 */
var Enums;
(function (Enums) {
        ...
        if (!guid.match(".*[a-zA-Z].*") && Number(guid) <= 0) {
            ...
            if (!AssetUtil.assetLoaded(guid)) {
                AssetUtil.asyncDownloadAsset(guid).then(() => {
                    ...
                    GameObject.asyncSpawn(guid, { transform: new Transform(new Vector(0, 0, -1000), rotate, scale) }).then((equipItem) => {
                        {
...
], MyClearAct.prototype, "onHitDamage", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct.prototype, "serverPlayEffect", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct.prototype, "serverPlayAnimotion", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct.prototype, "serverPlay3DSound", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct.prototype, "yuanPanding", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct.prototype, "boxPanding", null);
__decorate([
    RemoteFunction(mw.Client)
], MyClearAct.prototype, "boxPandingShow", null);
__decorate([
    RemoteFunction(mw.Client)
], MyClearAct.prototype, "yuanPandingShow", null);
__decorate([
    RemoteFunction(mw.Client)
], MyClearAct, "actNow", null);
__decorate([
    RemoteFunction(mw.Client)
], MyClearAct, "stopNow", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct, "serverEquipMe", null);
__decorate([
    RemoteFunction(mw.Server)
], MyClearAct, "serverDetachAllEquip", null);
MyClearAct = MyClearAct_1 = __decorate([
    Component
], MyClearAct);
...
var foreign47 = /*#__PURE__*/Object.freeze({
    __proto__: null,
    get DongXiaoList () { return DongXiaoList; },
    default: MyClearAct$1
});

/*
 * @Description: Prefab 通用协议
 */
var PrefabEvent;
...
__decorate([
    Decorator.noReply()
], WorldRankModuleS.prototype, "net_setFirst", null);
...
class PlayerModuleS extends ModuleS {
    ...
    enemyAtkPlayer(gameObjectIds, atk) {
        if (gameObjectIds == null || gameObjectIds == undefined || gameObjectIds.length == 0)
            return;
        gameObjectIds.forEach(gameObjectId => {
            if (!Helper.playerMap.has(gameObjectId))
                return;
    ...
    playerAtkPlayer(senderGuid, targetGuid, damage, hitPoint) {
        if (Helper.playerMap.has(senderGuid) && Helper.playerMap.has(targetGuid)
            ...
            if (Helper.teamMap.get(senderGuid) != Helper.teamMap.get(targetGuid)) {
                if (!hitPoint)
                    hitPoint = targeter.character.worldTransform.position;
    ...
    updatePlayerData(sendPlayer, targetPlayer, damage, hitPoint) {
        ...
        if (!this.playerDataSMap.has(playerId))
            return;
        ...
        if (curHp <= 0) {
            ...
            if (sendPlayer)
                this.playerKillEnemy(sendPlayer, 1, true);
    ...
    playerKillEnemy(sendPlayer, killCount, isPlayer) {
        ...
        if (!isPlayer)
            this.getAllClient().net_killTip(sendPlayer.userId, this.getWorldRankModuleS.getNameByUserId(sendPlayer.userId), "-1", (killCount > 1) ? "感染者" : Utils.getRoleName());
        try {
            if (this.playerDataSMap.get(sendPlayer.playerId).isDie == true)
                this.playerDataSMap.get(sendPlayer.playerId).isDie = false;
        }
        catch (error) {
        }
    }
    ...
    async bindPortalTrigger() {
        ...
        trigger.onEnter.add((char) => {
            if (Helper.playerMap.has(char.gameObjectId))
                this.portal(Helper.playerMap.get(char.gameObjectId));
        });
    }
    portal(player) {
        TimeUtil.delaySecond(2).then(() => {
            if (this.playerDataSMap.has(player.playerId))
                this.playerDataSMap.get(player.playerId).isDie = false;
        });
        let playerId = player.character.gameObjectId;
        let index = Helper.teamMap.has(playerId) ? Helper.teamMap.get(playerId) - 1 : 0;
        Console.error("index = " + index);
...
__decorate([
    Decorator.noReply()
], PlayerModuleS.prototype, "net_onEnterScene", null);
__decorate([
    Decorator.noReply()
], PlayerModuleS.prototype, "net_enemyAtkPlayer", null);
__decorate([
    Decorator.noReply()
], PlayerModuleS.prototype, "net_setCoin", null);
__decorate([
    Decorator.noReply()
], PlayerModuleS.prototype, "net_setDiamond", null);
class PlayerDataS {
    constructor() {
        this.isDie = true;
        this.maxHp = 0;
...
class ATKModuleC extends ModuleC {
    ...
    onStart() {
        ...
        InputUtil.onKeyDown(mw.Keys.G, () => {
            ...
            i++;
            if (i > GameConfig.Skill.getAllElement().length)
                i = 1;
        });
    }
    initDatas() {
    ...
    bindAction() {
        ...
        this.onAtkAction.add(() => {
            ...
            TimeUtil.delaySecond(this.skill.SkillCD).then(() => {
                ...
                if (this.skill.IsHasWeapon)
                    MyClearAct$1.serverDetachAllEquip(this.localPlayer);
            });
        });
    }
}
...
class HUDPanel extends HUDPanel_Generate$1 {
    ...
    bindSetButton() {
        this.mSetButton.onClicked.add(() => {
            if (this.mSetCanvas.visibility == mw.SlateVisibility.SelfHitTestInvisible)
                return;
            this.mSetCanvas.visibility = mw.SlateVisibility.SelfHitTestInvisible;
        });
        this.mSetCloseButton.onClicked.add(() => {
            if (this.mSetCanvas.visibility == mw.SlateVisibility.Collapsed)
                return;
            this.mSetCanvas.visibility = mw.SlateVisibility.Collapsed;
...
class GunModuleC extends ModuleC {
    ...
    buyGun(gunId, isDirectUse) {
        ...
        if (isDirectUse)
            this.setCurrentGunId(gunId);
        this.buyGunOnComplete();
        if (GameConfig.Gun.getElement(gunId).GunType == 2)
            return;
...
BuyGunPanel_Generate = __decorate([
    UIBind('UI/Gun/BuyGunPanel.ui')
], BuyGunPanel_Generate);
...
class BuyGunPanel extends BuyGunPanel_Generate$1 {
    ...
    bindButtons() {
        ...
        this.mDiamondButton.onClicked.add(() => {
            if (this.gunModuleC.isBuyGunByDiamond(this.gunId, this.isDirectUse))
                this.hide();
        });
        this.mCoinButton.onClicked.add(() => {
            if (this.gunModuleC.isBuyGunByCoin(this.gunId, this.isDirectUse))
                this.hide();
        });
    }
...
class GunPanel extends GunPanel_Generate$1 {
    ...
    updateSelectGun(gunId) {
        ...
        if (this.currentGunId != 0)
            this.gunItmeMap.get(this.currentGunId).unselect();
        this.currentGunId = gunId;
...
class LotteryPanel extends LotteryPanel_Generate$1 {
    ...
    bindButtons() {
        ...
        this.mCoinStaleButton.onClicked.add(() => {
            ...
            if (this.lotteryModuleC.isCanCoinLottery(18888))
                this.startLottery();
        });
        this.mDiamondStaleButton.onClicked.add(() => {
            if (this.rewardList.length <= 0) {
            ...
            if (this.lotteryModuleC.isCanDiamondLottery(2))
                this.startLottery();
        });
    }
    onShow() {
    ...
    startLottery() {
        if (this.preGunId)
            this.lotteryItems[this.preGunId - 1].setSelectState(false);
...
BuyRolePanel_Generate = __decorate([
    UIBind('UI/BuyRole/BuyRolePanel.ui')
], BuyRolePanel_Generate);
...
__decorate([
    Decorator.persistence()
], RoleData.prototype, "roleIds", void 0);
__decorate([
    Decorator.persistence()
], RoleData.prototype, "currentRoleId", void 0);
...
class RolePanel extends RolePanel_Generate$1 {
    ...
    bindButtons() {
        ...
        this.mLeftButton.onClicked.add(() => {
            if (--this.currentSelectRoleIndex < 0)
                this.currentSelectRoleIndex = this.maxRoleCount - 1;
        ...
        this.mRightButton.onClicked.add(() => {
            if (++this.currentSelectRoleIndex >= this.maxRoleCount)
                this.currentSelectRoleIndex = 0;
            this.pickUpRoleModuleC.onUpdateCameraPos.call(this.currentSelectRoleIndex);
            this.updateUseButtonInfo(this.pickUpRoleModuleC.isHasRoleId(this.currentSelectRoleIndex + 1));
        });
...
class PickUpRoleModuleC extends ModuleC {
    ...
    async findRoleBindTriggers() {
        for (let i = 0; i < this.roleElements.length; ++i) {
            if (!this.roleElements[i].Trigger)
                continue;
            ...
            if (cameraAnchor)
                this.cameraAnchors.push(cameraAnchor.worldTransform);
...
__decorate([
    Decorator.noReply()
], PickUpRoleModuleS.prototype, "net_buyRole", null);
__decorate([
    Decorator.noReply()
], PickUpRoleModuleS.prototype, "net_setCurrentRoleId", null);
__decorate([
    Decorator.noReply()
], PickUpRoleModuleS.prototype, "net_setDescription", null);
...
InputPanel_Generate = __decorate([
...
class PlayerModuleC extends ModuleC {
    ...
    net_syncOneTeamData(teamId, playerId, playerName) {
        if (this.localPlayer.character.gameObjectId == playerId)
            return;
        Helper.teamMap.set(playerId, teamId);
        let teamDatas = [];
    ...
    net_changeTeam(playerId, curTeam, toTeam) {
        ...
        if (!curTeamDatas || curTeamDatas.length == 0)
            curTeamDatas = null;
...
__decorate([
    Decorator.noReply()
], GuideModuleS.prototype, "net_setFirst", null);
...
__decorate([
    Decorator.persistence()
], GunData.prototype, "gunIds", void 0);
__decorate([
    Decorator.persistence()
], GunData.prototype, "currentGunId", void 0);
...
__decorate([
    Decorator.noReply()
], GunModuleS.prototype, "net_buyGun", null);
__decorate([
    Decorator.noReply()
], GunModuleS.prototype, "net_setCurrentGunId", null);
__decorate([
    Decorator.noReply()
], GunModuleS.prototype, "net_playerSwitchGun", null);
...
LoginPanel_Generate = __decorate([
...
__decorate([
    Decorator.persistence()
], LoginData.prototype, "days", void 0);
__decorate([
    Decorator.persistence()
], LoginData.prototype, "isGet", void 0);
class LoginModuleC extends ModuleC {
    ...
    onEnterScene(sceneType) {
        ...
        if (this.days >= 3)
            this.loginPanel.setConditionImage1Visible(true);
    }
    isCanGet() {
        return (this.days >= 3);
    }
...
__decorate([
    Decorator.noReply()
], LoginModuleS.prototype, "net_setAddLogin", null);
__decorate([
    Decorator.noReply()
], LoginModuleS.prototype, "net_setIsGet", null);
...
class RadarModuleC extends ModuleC {
    ...
    isFriendly(player1, player2) {
        if (Math.abs(Helper.teamMap.get(player1.character.gameObjectId) -
            Helper.teamMap.get(player2.character.gameObjectId)) == 0)
            return true;
        return false;
    }
    /**
...
WorldRankPanel_Generate = __decorate([
    UIBind('UI/WorldRank/WorldRankPanel.ui')
...
class WorldRankModuleC extends ModuleC {
    ...
    initRankNpcAnim() {
        TimeUtil.setInterval(async () => {
            ...
            if (randomNpc?.currentAnimation?.isPlaying)
                randomNpc.currentAnimation?.stop();
            if (!mw.AssetUtil.assetLoaded(randomAnim))
                await mw.AssetUtil.asyncDownloadAsset(randomAnim);
    ...
    async updateRankNpc() {
        ...
        for (let i = 0; i < userIds.length; ++i) {
            ...
            if (!this.rankNpcs[i] || !tmpChar.character)
                continue;
    ...
    setCharVisibility(char, visibility) {
        if (char.getVisibility() != visibility)
            char.setVisibility(visibility, true);
    }
}
/**排行类型 */
var RankType;
...
let SP_SoundMoveBoard = class SP_SoundMoveBoard extends mw.Script {
    constructor() {
        ...
        this.number_soundDistance = 1000;
        //触发器
...
CubeLifebar_Generate = __decorate([
    UIBind('UI/Enemy/CubeLifebar.ui')
], CubeLifebar_Generate);
...
let Boss = class Boss extends Script {
    ...
    playerAtkEnemyS(senderGuid, targetGuid, damage, hitPoint) {
        ...
        if (this.curHp <= 0) {
            this.dieS();
            if (Helper.playerMap.has(senderGuid))
                this.getPlayerModuleS.playerKillEnemy(Helper.playerMap.get(senderGuid), ((this.maxHp == 100) ? 1 : 5), false);
        ...
        if (Helper.playerMap.has(senderGuid))
            this.getPlayerModuleS.playerAtkEnemyFlyText(Helper.playerMap.get(senderGuid), hitPoint, damage);
    }
    dieS() {
...
let Enemy_Cube = class Enemy_Cube extends Script {
    ...
    playerAtkEnemyS(senderGuid, targetGuid, damage, hitPoint) {
        ...
        if (this.curHp <= 0) {
            ...
            this.dieS();
            if (Helper.playerMap.has(senderGuid))
                this.getPlayerModuleS.playerKillEnemy(Helper.playerMap.get(senderGuid), 5, false);
            TimeUtil.delaySecond(this.respawnTime).then(() => {
    ...
    meleeAttackS() {
        ...
        for (let i = 0; i < targets.length; ++i) {
            ...
            if (target == null || target == undefined || !(target instanceof mw.Character))
                continue;
...
let EnemyBullet = class EnemyBullet extends Script {
    ...
    onTriggerEnterS(other) {
        if (other.tag == "self" || other.tag == "bullet")
            return;
        this.reclcleS();
...
WeaponUI_Generate = __decorate([
    UIBind('UI/Prefabs/步枪/UI/WeaponUI.ui')
], WeaponUI_Generate);
...
let WeaponDriver = WeaponDriver_1 = class WeaponDriver extends mw.Script {
                ...
                this.clientOnHit = (hitResult, attackPlayer, isObj) => {
                    hitResult.forEach(e => {
                        if (e instanceof mw.HitResult) {
                            if (PlayerManagerExtesion.isCharacter(e.gameObject) ||
    ...
    hitCharaPerformance(loc, rot) {
        ...
        try {
            if (this.hitCharaEffect)
                GeneralManager.rpcPlayEffectAtLocation(this.hitCharaEffect.assetId, loc, 1, rot, this.hitCharaEffect.worldTransform.scale);
            if (this.hitCharaSound)
                SoundService.play3DSound(this.hitCharaSound.assetId, loc, 1, 1, { falloffDistance: 3000 });
        }
        catch (error) {
    ...
    hitObjectPerformance(loc, rot) {
        ...
        try {
            if (this.hitEffect)
                GeneralManager.rpcPlayEffectAtLocation(this.hitEffect.assetId, loc, 1, rot, this.hitEffect.worldTransform.scale);
            if (this.hitSound)
                SoundService.play3DSound(this.hitSound.assetId, loc, 1, 1, { falloffDistance: 3000 });
        }
        catch (error) {
...
WeaponDriver = WeaponDriver_1 = __decorate([
    Component
], WeaponDriver);
...
RankItem_Generate = __decorate([
    UIBind('UI/WorldRank/RankItem.ui')
], RankItem_Generate);
...
WorldRankItem_Generate = __decorate([
    UIBind('UI/WorldRank/WorldRankItem.ui')
...
const MWFileMapping = new WeakMap([[foreign1 || {}, "JavaScripts/common/Dance"],
[foreign2 || {}, "JavaScripts/common/FlyText"],
[foreign3 || {}, "JavaScripts/common/Guide"],
...
[foreign8 || {}, "JavaScripts/config/ConfigBase"],
[foreign9 || {}, "JavaScripts/config/GameConfig"],
[foreign10 || {}, "JavaScripts/config/Gun"],
[foreign11 || {}, "JavaScripts/config/PickUpGun"],
[foreign12 || {}, "JavaScripts/config/Role"],
[foreign13 || {}, "JavaScripts/config/Skill"],
[foreign14 || {}, "JavaScripts/config/Task"],
[foreign15 || {}, "JavaScripts/const/Enum"],
[foreign16 || {}, "JavaScripts/const/Globaldata"],
[foreign17 || {}, "JavaScripts/const/Helper"],
[foreign18 || {}, "JavaScripts/GameLauncher"],
[foreign19 || {}, "JavaScripts/Interaction/SP_SoundMoveBoard"],
[foreign20 || {}, "JavaScripts/Modified027Editor/ModifiedCamera"],
[foreign21 || {}, "JavaScripts/Modified027Editor/ModifiedPlayer"],
[foreign22 || {}, "JavaScripts/Modified027Editor/ModifiedSpawn"],
[foreign23 || {}, "JavaScripts/Modified027Editor/ModifiedStaticAPI"],
[foreign24 || {}, "JavaScripts/module/AdsModule/AdsModuleC"],
[foreign25 || {}, "JavaScripts/module/AdsModule/AdsModuleS"],
[foreign26 || {}, "JavaScripts/module/AdsModule/ui/AdTipsPanel"],
[foreign27 || {}, "JavaScripts/module/Enemy/Boss"],
[foreign28 || {}, "JavaScripts/module/Enemy/Enemy"],
[foreign29 || {}, "JavaScripts/module/Enemy/EnemyBullet"],
[foreign30 || {}, "JavaScripts/module/Enemy/EnemyModule"],
[foreign31 || {}, "JavaScripts/module/GuideModule/GuideModule"],
[foreign32 || {}, "JavaScripts/module/GunModule/GunData"],
[foreign33 || {}, "JavaScripts/module/GunModule/GunModuleC"],
[foreign34 || {}, "JavaScripts/module/GunModule/GunModuleS"],
[foreign35 || {}, "JavaScripts/module/GunModule/LoginModule"],
[foreign36 || {}, "JavaScripts/module/GunModule/PickUpGunModule"],
[foreign37 || {}, "JavaScripts/module/GunModule/ui/BuyGunPanel"],
[foreign38 || {}, "JavaScripts/module/GunModule/ui/GunPanel"],
[foreign39 || {}, "JavaScripts/module/HUDModule/ATKModule"],
[foreign40 || {}, "JavaScripts/module/HUDModule/HUDData"],
[foreign41 || {}, "JavaScripts/module/HUDModule/HUDModuleC"],
[foreign42 || {}, "JavaScripts/module/HUDModule/HUDModuleS"],
[foreign43 || {}, "JavaScripts/module/HUDModule/ui/HUDPanel"],
[foreign44 || {}, "JavaScripts/module/HUDModule/ui/KillTipItem"],
[foreign45 || {}, "JavaScripts/module/LotteryModule/LotteryModule"],
[foreign46 || {}, "JavaScripts/module/PickUpRoleModule/PickUpRoleModule"],
[foreign47 || {}, "JavaScripts/module/PlayerModule/MyClearAct"],
[foreign48 || {}, "JavaScripts/module/PlayerModule/PlayerData"],
[foreign49 || {}, "JavaScripts/module/PlayerModule/PlayerModuleC"],
[foreign50 || {}, "JavaScripts/module/PlayerModule/PlayerModuleS"],
[foreign51 || {}, "JavaScripts/module/PlayerModule/ui/InputPanel"],
[foreign52 || {}, "JavaScripts/module/PlayerModule/ui/TeamPanel"],
[foreign53 || {}, "JavaScripts/module/RadarModule/RadarModuleC"],
[foreign54 || {}, "JavaScripts/module/RadarModule/RadarModuleS"],
[foreign55 || {}, "JavaScripts/module/RadarModule/ui/RadarUI"],
[foreign56 || {}, "JavaScripts/module/RankModule/PlayerPropData"],
[foreign57 || {}, "JavaScripts/module/RankModule/ui/WorldRankPanel"],
[foreign58 || {}, "JavaScripts/module/RankModule/WorldRankModuleC"],
[foreign59 || {}, "JavaScripts/module/RankModule/WorldRankModuleS"],
...
[foreign70 || {}, "JavaScripts/tools/TouchScript"],
[foreign71 || {}, "JavaScripts/tools/utils"],
[foreign72 || {}, "JavaScripts/ui-generate/Ads/AdsTipsPanel_generate"],
[foreign73 || {}, "JavaScripts/ui-generate/BuyRole/BuyRolePanel_generate"],
[foreign74 || {}, "JavaScripts/ui-generate/BuyRole/RolePanel_generate"],
[foreign75 || {}, "JavaScripts/ui-generate/Enemy/CubeLifebar_generate"],
[foreign76 || {}, "JavaScripts/ui-generate/Gun/BuyGunPanel_generate"],
[foreign77 || {}, "JavaScripts/ui-generate/Gun/GunItem_generate"],
[foreign78 || {}, "JavaScripts/ui-generate/Gun/GunPanel_generate"],
[foreign79 || {}, "JavaScripts/ui-generate/Gun/LoginPanel_generate"],
[foreign80 || {}, "JavaScripts/ui-generate/HUD/HUDPanel_generate"],
[foreign81 || {}, "JavaScripts/ui-generate/HUD/KillTipItem_generate"],
[foreign82 || {}, "JavaScripts/ui-generate/Lottery/LotteryItem_generate"],
[foreign83 || {}, "JavaScripts/ui-generate/Lottery/LotteryPanel_generate"],
[foreign84 || {}, "JavaScripts/ui-generate/notice/NoticeView_generate"],
[foreign85 || {}, "JavaScripts/ui-generate/notice/SecondNoticeItem_generate"],
[foreign86 || {}, "JavaScripts/ui-generate/notice/TopNoticeItem_generate"],
[foreign87 || {}, "JavaScripts/ui-generate/Player/InputPanel_generate"],
[foreign88 || {}, "JavaScripts/ui-generate/Portal_generate"],
[foreign89 || {}, "JavaScripts/ui-generate/Prefabs/步枪/UI/WeaponUI_generate"],
[foreign90 || {}, "JavaScripts/ui-generate/Task/TaskItem_generate"],
[foreign91 || {}, "JavaScripts/ui-generate/Task/TaskPanel_generate"],
[foreign92 || {}, "JavaScripts/ui-generate/Team/TeamItem_generate"],
[foreign93 || {}, "JavaScripts/ui-generate/Team/TeamPanel_generate"],
[foreign94 || {}, "JavaScripts/ui-generate/WorldRank/RankItem_generate"],
[foreign95 || {}, "JavaScripts/ui-generate/WorldRank/WorldRankItem_generate"],
[foreign96 || {}, "JavaScripts/ui-generate/WorldRank/WorldRankPanel_generate"]]);
